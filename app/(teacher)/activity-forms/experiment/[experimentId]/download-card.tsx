...
interface ExperimentDownloadCardProps {
  experimentId: string;
  experimentName: string;
  gradeName: string;
}

export function ExperimentDownloadCard({
  experimentId,
  experimentName,
  gradeName,
}: ExperimentDownloadCardProps) {
  ...
  const handleDownload = async (type: "student" | "teacher") => {
    ...
    try {
      ...
      const response = await fetch(
        `/api/experiments/${experimentId}/export?${params.toString()}`
      );
      ...
      a.href = url;
      a.download = `${experimentName.replace(/[^a-zA-Z0-9]/g, "-").toLowerCase()}-${type}-copy.pdf`;
