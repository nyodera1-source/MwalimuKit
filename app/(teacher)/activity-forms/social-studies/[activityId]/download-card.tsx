...
interface SocialStudiesDownloadCardProps {
  activityId: string;
  activityName: string;
  gradeName: string;
}

export function SocialStudiesDownloadCard({
  activityId,
  activityName,
  gradeName,
}: SocialStudiesDownloadCardProps) {
  ...
  const handleDownload = async (type: "student" | "teacher") => {
    ...
    try {
      ...
      const response = await fetch(
        `/api/social-studies/${activityId}/export?${params.toString()}`
      );
    ...
    <Card className="border-2 border-indigo-200 dark:border-indigo-900/50 shadow-lg">
      ...
      <CardContent className="space-y-4">
        ...
        <p className="text-xs text-muted-foreground text-center pt-1">
          Teacher Guide includes expected outcomes & discussion guidance
        </p>
      </CardContent>
    </Card>
  );
